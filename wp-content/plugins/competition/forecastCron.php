...
require_once('functions/Quotes.php');
...
try {  
    ...
    $result = $mysqli->query("SELECT * FROM wp_competition_base WHERE is_done is null OR is_done = 0");   
    ...
    //прогноз
    $quotesClass = new Quotes();
    $quotes = $quotesClass->get();
    if (empty($quotes)) {
        throw new QuotesException('no quotes');       
    }
    $bid = $quotes['EURUSD']['bid'];    
    //$difference = (mt_rand(0, 200)-100)/10000;
    $difference = (mt_rand(0, 60)-30)/100000;
    ...
    if ($statusAnswer === 1) {
        ...
        $result = $mysqli->query("UPDATE wp_competition_base set is_done = 1 where id = $participant->id");
        return false;
    }
