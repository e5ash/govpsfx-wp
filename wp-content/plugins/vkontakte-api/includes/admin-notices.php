...
/**
 * Выводит реламные уведомления в админке
 *
 * @package WordPress
...
class Wbcr_VkApi_Admin_Notices {
        ...
        if( in_array(get_locale(), array('ru_RU', 'bel', 'kk', 'uk', 'bg', 'bg_BG', 'ka_GE')) ) {
            ...
            $this->suggestions = array(
                'robin-image-optimizer' => array(
                    ...
                    'ico' => WVAI_PLUGIN_URL . '/images/robin.jpg'
                ),
            );
        } else {
            $this->suggestions = array(
    ...
    public function load_notices_suggestions()
    {
        ...
        if( $this->disable_nag_notices() ) {
			...
			if( !empty($this->missing) && is_array($this->missing) ) {
                wp_enqueue_style('wbcr_pvkapi-notices', WVAI_PLUGIN_URL.'/assets/css/notices.css');
    ...
    public function admin_notices_suggestions()
    {
        ...
        ?>
        <div class="<?php echo esc_attr($this->prefix); ?>-dismiss-suggestions wbcr_pvkapi-notice-wrapper notice notice-success is-dismissible" data-nonce="<?php echo esc_attr(wp_create_nonce($this->prefix . '-dismiss-suggestions')); ?>">
            ...
            <?php foreach($this->missing as $plugin) : ?>
                ...
                <div class='wbcr_pvkapi-notice'>
                    <div class='wbcr_pvkapi-leftcol'><img class='wbcr_pvkapi-plugin-ico' src='<?php echo $this->suggestions[$plugin]['ico']; ?>' alt='<?php echo $plugin; ?>'></div>
                    <div class='wbcr_pvkapi-content'>
                        <div class='wbcr_pvkapi-title'><?php echo $this->suggestions[$plugin]['name']; ?></div>
                        <div class="wbcr_pvkapi-text-cols">
                            <div class='wbcr_pvkapi-description'><?php echo $this->suggestions[$plugin]['desc']; ?></div>
                            <div class='wbcr_pvkapi-rightcol'>
                                <a class='wbcr_pvkapi-install-btn' href='<?php echo esc_url($this->get_install_url($plugin)); ?>'>
