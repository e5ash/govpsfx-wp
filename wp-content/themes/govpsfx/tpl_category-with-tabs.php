<?php
/*
Template Name: Категории с вкладками
...
<div class="page bg-wrap">
			...
			<div class="page__wrap row">
				<div class="page__sidebar">
					<div class="page__sticky">
						<div class="page__nav-drop nav-drop">
							<?php $ids = get_ancestors(get_the_ID(), 'page');
							$id  = array_pop($ids);
						?>
							<?php foreach(children_pages($id) as $page) : ?>
							<?php if (children_pages($page->ID)) : ?>
							<div class="nav-drop__item tog <?= $post->post_parent == $page->ID ? '--show' : null ?>">
								<div class="nav-drop__head tog-head">
									<div class="nav-drop__title"><?= $page->post_title; ?></div>
									<div class="nav-drop__icon"></div>
								</div>
								<div class="nav-drop__body tog-body">
									<ul>
										<?php foreach(children_pages($page->ID) as $ch_page) : ?>
										<li class="<?= $ch_page->ID == get_the_ID() ? '--selected' : ''?>"><a href="<?= the_permalink($ch_page->ID); ?>" target="_blank">
											<?= $ch_page->post_title; ?>		
										</a></li>
										<?php endforeach; ?>
									</ul>
	...
	<div class="combo-offer overflow-disable bg-wrap --inner">
		<div class="case --sm">
