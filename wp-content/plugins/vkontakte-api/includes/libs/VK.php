...
class WVK_Connect {
	 ...
	 * @return  array
	 */
	public function getAccessToken($callback_url = 'https://api.vk.com/blank.html')
	{
		if( !is_null($this->access_token) && $this->auth ) {
			throw new WVKException('Already authorized.');
		}
		...
		$rs = $this->request($this->createUrl(self::ACCESS_TOKEN_URL, $parameters));
		$rs = json_decode($rs['body'], true);

		if( isset($rs['error']) ) {
			throw new WVKException($rs['error'] . (!isset($rs['error_description'])
	...
	public function api($method, $parameters = array(), $format = 'array', $requestMethod = 'get')
	{
		...
		if( $method == 'execute' || $requestMethod == 'post' ) {
			$rs = $this->request($this->getApiUrl($method, $format == 'array'
				? 'json'
				: $format), "POST", $parameters);
		} else {
			$rs = $this->request($this->createUrl($this->getApiUrl($method, $format == 'array'
				? 'json'
				: $format), $parameters));
		}

		return $format == 'array'
			? json_decode($rs['body'], true)
			: $rs;
	}
