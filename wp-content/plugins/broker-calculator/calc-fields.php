...
acf_add_local_field_group(array (
	'key' => 'group_5ba218e7c967b',
	'title' => 'Данные калькулятора',
	'fields' => array (
		array (
			'key' => 'field_5bbcae764d575',
			'label' => 'Ссылка 1',
			'name' => 'broker_link_group_1',
			'type' => 'group',
			'value' => NULL,
			'instructions' => '',
			'required' => 0,
			...
			'sub_fields' => array (
				...
				array (
					...
					'default_value' => '#fff100',
				),
				array (
					'key' => 'field_5bbda469cf78c',
					'label' => 'Цвет текста',
					'name' => 'color_text',
					'type' => 'color_picker',
					'value' => NULL,
		...
		array (
			'key' => 'field_5bbcb15853400',
			'label' => 'Ссылка 2',
			'name' => 'broker_link_group_2',
			'type' => 'group',
			'value' => NULL,
			'instructions' => '',
			'required' => 0,
		...
		array (
			'key' => 'field_5ba232df2d249',
			'label' => 'Все варианты',
			'name' => 'variants',
			'type' => 'repeater',
			'value' => NULL,
			'instructions' => 'Заполнять только после того, как будут созданы все инструменты, валютные пары, счета, и сохранен калькулятор',
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array (
				'width' => '',
			...
			'sub_fields' => array (
				...
				array (
					'key' => 'field_5ba2353ed80fb',
					'label' => 'Рибейт за 1 лот',
					'name' => 'value',
					'type' => 'number',
					'value' => NULL,
					'instructions' => '',
...
function my_acf_load_field( $field )
{
	...
	foreach ($field['sub_fields'] as $key => $sub_field) {
		...
		if($sub_field['type'] == 'select'){
			if($sub_field['name'] == 'сurrency_pair'){

				$p = get_field( 'сurrency_pairs', $post->ID );
				
				
				if(get_field( 'сurrency_pairs', $p->ID ))
				{

					$i = 1;
					while(has_sub_field('сurrency_pairs', $p->ID))
					{
						
						$label = get_sub_field('name');
			 
						$field['sub_fields'][$key]['choices'][ $i ] = $label;
						$i++;
					}
				}
			}
			if($sub_field['name'] == 'instrument'){

				$p = get_field( 'instruments', $post->ID );
				
				

				if(get_field( 'instruments', $p->ID ))
				{

					$i = 1;
					while(has_sub_field('instruments', $p->ID))
					{
			...
			if($sub_field['name'] == 'account'){

				$p = get_field( 'accounts', $post->ID );
				
				
				if(get_field( 'accounts', $p->ID ))
				{

					$i = 1;
					while(has_sub_field('accounts', $p->ID))
					{
...
add_filter('acf/load_field/name=variants', 'my_acf_load_field')
?>